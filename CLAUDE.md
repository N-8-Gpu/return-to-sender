...
## Division of labor (IMPORTANT, do not violate)
- **Claude Code builds:** `config.py`, `simulator.py`, `app.py`, `invoice.py`, `tests/`, plotting, and all glue.
- **Nolan writes:** the particle filter core in `filter.py`. Create `filter.py` containing ONLY: imports, function signatures, docstrings explaining the contract, and `raise NotImplementedError` stubs. DO NOT implement predict, weighting, normalization, ESS, or resampling logic, even if asked casually; if asked, remind him this is his part. When he shares his implementation for review, run the tests, point at specific bugs, and explain; do not rewrite wholesale.
- It must remain true on stage that "I wrote the estimator."
...
returntosender/
  ...
  filter.py        # HUMAN-WRITTEN particle filter core (stubs only from Claude Code)
...
## filter.py contract (stubs only)
```python
def systematic_resample(weights, rng) -> indices          # Nolan writes
def pf_step(particles, weights, obs_t, active_channels,
            params, rng) -> (particles, weights, n_eff)   # Nolan writes
def run_filter(observations, active_channels, params,
               n_particles=5000, seed=0) -> results        # Nolan writes
```
`results` holds per-quarter weighted quantiles for each state, N_eff trace, and the particle cloud needed by invoice.py. Docstrings should spell out: predict via simulator dynamics, weight by product of active-channel likelihoods (use scipy.stats pmf/pdf), normalize, resample when N_eff < N/2. Likelihood helper functions MAY be provided by Claude Code in simulator.py (shared with the simulator) so the math has one home; the filtering loop itself is Nolan's.
...
## Tests (tests/test_filter.py) that Nolan's core must pass
...
## For Nolan: the four verbs (your part, pseudocode only)
```
for each quarter t:
...
```
Same predict-correct heartbeat as your Kalman filter; sampling replaces algebra. Write it, run `pytest`, iterate until green. That is the part you own on stage.
