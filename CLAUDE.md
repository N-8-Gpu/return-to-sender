...
## app.py (Streamlit)
Framed as a consulting engagement: "Return to Sender Advisory" (fictional, clearly labeled) prepared for a provincial stewardship authority (illustrative client). Single page (no multi-page apps), five tabs: Executive brief (KPI metrics, avoided-externality counterfactual when the deposit is on, cumulative-cost exhibit) / Evidence & estimator (shrinking-bands chart, 4-beat demo script, brand mass-balance roadmap) / The invoice (figure + PNG download + unit-cost sensitivity sweep) / Legal pathway (EPR landscape from SOURCES tags; open questions labeled) / Methodology & sources. Sidebar: data-source radio (demo scenario vs client CSV upload with downloadable template and validation), evidence-program checkboxes with real-world-analog tooltips, tags tau slider, deposit toggle + quarter slider, seed, Run button. Results persist in session_state across widget changes (stale-settings warning); engagement runs are st.cache_data-cached; page body lives in main() gated on runtime.exists() so importing app.py is side-effect-free for tests. Everything reruns in under ~2 seconds on a laptop (5000 particles x 40 quarters is milliseconds; keep it vectorized).
